...
def bus_stop():
    ...
    #     if stop != 0:
    #         result_list.append(stop) # stop_list에서 0을 제거한다.

    """
    지나간 버스가 없는 정류장(0에 해당하는 정류장) 도 동일하게 출력해야합니다.
    그런데 5001개의 모든 버정을 출력할 필요는 없으므로, 문제에서 주어진 버스정류장에 한해서만 출력하면 됨으로
    문제에서 주어진 버스정류장(bus_stop_list)만 결과에 추가해준 뒤 출력하면 됩니다.
    """
    for stop in bus_stop_list:
        # if stop != 0:
        result_list.append(stop_list[stop]) # stop_list에서 0을 제거한다.
